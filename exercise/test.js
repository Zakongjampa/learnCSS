...
const util = {
    ...
    filterByMinRating: function (rating) {
        return function (singleFeedBack) {
...
const another_sorted = numbers.sort((first, second) =>{
    return first - second;
});

const decreased_order = numbers.sort((f,s) =>{
    return s - f;
})
...
// console.table(feedback.filter(util.findByWord('burg')))
// let filterRatingTwo = util.filterByMinRating(2)

// let filterRatingThree = util.filterByMinRating(3);

// console.table(feedback.filter(filterRatingTwo));
// console.table(feedback.filter(filterRatingThree));

// let BurgFinder = util.findByWord('burg');
// let SmoothiesFinder = util.findByWord('Smoothies')
// const burgRating = feedback.find(BurgFinder);
// const smoothiesRating = feedback.find(SmoothiesFinder);

// let burgerRating = feedback.filter(BurgFinder);
// console.table(burgRating);

// console.log(burgRating);
// console.log(smoothiesRating);
...
let clean_people = people.map(person =>{
    let now = Date.now();

    let bd = new Date(person.birthday).getTime(); 
    let age = Math.floor((now - bd) / 31536000000 );
    return {
        age,
...
This is doing the same job as the
<link> element for CSS. It applies the JavaScript to the page, so it can have an effect on the HTML (along with the CSS,
...
Make sure the HTML and JavaScript files are saved. Then load index.html in your browser. You should see something like
this:

Heading "hello world" above a firefox logo

Note: The reason the instructions (above) place the
<script> element near the bottom of the HTML file is that the browser reads code in the order it appears in the file.
...
// let count_occurence = text.split("").filter(ch => /^[0-9a-zA-Z]+$/.test(ch)).map(ch => ch.toLowerCase()).reduce((acc, item)=>{
...
for(let fab of family){
    console.log(fab);
}

for(let fab in family){
    console.log(family[fab])
}

let dash = [...family];

dash.forEach(e=> console.log(e));